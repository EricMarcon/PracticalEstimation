...
q.seq <-  seq(0, 2, by=.1)        # c(seq(0, .1, 0.025), .15, seq(.2, .6, 0.1))
NumberOfSimulations <- 100        # 1000
Alpha <- 0.05
```
...
```{r RMSE, fig.cap="(ref:RMSE)", out.width = '90%'}
# Lognormal
RMSE <- tibble(q=q.seq)
for (Correction in Corrections) {
  RMSE <- add_column(
...
grid.arrange(
  ggRMSE_Elnorm300_1000 + theme(legend.position = "none"), 
  ggRMSE_Egeom300_1000 + theme(legend.position = "none"),
  gg_legend, 
  nrow = 1, 
...
(ref:BCIProfile) Estimated diversity profile of the tree species of the BCI 50-ha plot. The shaded zone is the 95% confidence interval of the estimation.
